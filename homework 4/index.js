...
glossarys.forEach(function (glossary) {
  let tags = glossary.tags;
  let def = glossary.definition;
  let classNum = glossary.class;
...
function handleSubmit(event) {
  ...
  let glossary = target.getElementsByTagName("li");
  Array.from(glossary).forEach(function (data) {
    const title = data.textContent;
...
function getSelectValue() {
  let allValue = document.querySelector("#class").value;
  glossarys.filter(function (glossary) {
    let classOpt = glossary.class;
    if (classOpt.indexOf(allValue) == 0) {
      return true;
    } else if (classOpt.indexOf(allValue) == 1) {
      return classOpt == "1";
    } else if (classOpt.indexOf(allValue) == 2) {
      return classOpt == "2";
    } else if (allValue == 3) {
      return classOpt == "3";
    } else if (allValue == 4) {
